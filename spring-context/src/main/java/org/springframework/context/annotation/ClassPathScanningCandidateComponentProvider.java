...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	...
	* */
	...
	/**
	 * 扫描指定的包下所有的被spring注解的bean
	 * @param basePackage 指定要扫描的包
	 * @return 被扫描到的bean集合  实例为new ScannedGenericBeanDefinition(metadataReader);
	 */
		...
		try {
			...
			for (Resource resource : resources) {
				...
				try {
					...
					/*
					* 判断该类是否符合spring的bean条件（会根据配置排除或加入一些包），符合条件的放入candidates集合
					* */

					//getMetadataReader()会读取类的相关信息，如注解
					MetadataReader metadataReader = getMetadataReaderFactory().getMetadataReader(resource);
					//判断是否符合spring的bean，是的话加入到集合
					if (isCandidateComponent(metadataReader)) {
						ScannedGenericBeanDefinition sbd = new ScannedGenericBeanDefinition(metadataReader);
